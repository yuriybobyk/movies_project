...
const ComedyRow = ({title}:IProps) => {
    ...
    const {comedyMovies, isModalOpen} = useAppSelector(state => state.movieReducer)

    useEffect(()=>{
        dispatch(movieActions.getComedyMovies({page:'1'}))
    },[dispatch])
        ...
        <div className="h-80 space-y-0.5 md:space-y-2">
            ...
            <div className="group relative md:-ml-2">
                ...
                <div ref={rowRef} className="flex items-center space-x-0.5 overflow-x-scroll scrollbar-hide md:space-x-2.5 md:p-2">
                    {comedyMovies&& comedyMovies.map(comedyMovie=><RowElement onCardClick={handleMovieCardClick} movie={comedyMovie} key={comedyMovie.id}/>)}
                </div>
                <ChevronRightIcon
