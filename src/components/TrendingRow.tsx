...
const TrendingRow = ({title}: IProps) => {
    ...
    const {trending, isModalOpen, trailer} = useAppSelector(state => state.movieReducer)

    useEffect(() => {
        dispatch(movieActions.getTrending({page: '1'}))
    }, [dispatch]);
        ...
        <div className="h-80 space-y-0.5 md:space-y-2">
            ...
            <div className="group relative md:-ml-2">
                ...
                <div ref={rowRef}
                    ...
                    {trending && trending.map(trend => <RowElement onCardClick={handleMovieCardClick} movie={trend}
                                                                   key={trend.id}/>)}
                </div>
                <ChevronRightIcon
